...
export const createLinkToken = async (req, res, next) => {
  try {
    ...
    const response = await getPlaidClient().linkTokenCreate({
      user: {
        client_user_id: String(req.user._id),
      },
      client_name: 'FinDash',
    ...
    return res.status(400).json({
      ...
      plaidError: err.response?.data || null,
    });
  }
};
/**
...
export const syncTransactions = async (req, res, next) => {
  try {
    ...
    for (const item of user.plaidItems) {
      ...
      const txResponse = await getPlaidClient().transactionsGet({
