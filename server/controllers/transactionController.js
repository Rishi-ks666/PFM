...
import { convertTransaction } from '../utils/currencyConverter.js';
...
export const getTransactions = async (req, res, next) => {
  try {
    ...
    const convertedTransactions = transactions.map(tx => convertTransaction(tx.toObject(), targetCurrency, rates));

    return res.json({ transactions: convertedTransactions, page, totalPages, totalCount });
  } catch (err) {
    next(err);
  }
};
...
export const getTransactionById = async (req, res, next) => {
  try {
    ...
    return res.json(convertTransaction(transaction.toObject(), targetCurrency, rates));
  } catch (err) {
    next(err);
  }
};
...
export const updateTransaction = async (req, res, next) => {
  try {
    ...
    return res.json(convertTransaction(existingTx.toObject(), targetCurrency, rates));
  } catch (err) {
    next(err);
  }
};
