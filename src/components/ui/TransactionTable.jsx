              ...
              currentData.map((tx, idx) => (
                ...
                >
                  ...
                  <td className="py-3.5 px-4 text-right">
                    <div className={`text-sm font-bold font-mono tracking-tight ${tx.convertedAmount > 0 || tx.amount > 0 ? 'text-emerald-400' : 'text-slate-200'}`}>
                      {tx.convertedAmount > 0 || tx.amount > 0 ? '+' : ''}{formatCurrency(Math.abs(tx.convertedAmount || tx.amount))}
                    </div>
                    {tx.currency && tx.currency !== tx.displayCurrency && (
