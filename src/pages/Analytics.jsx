...
function SpendingHeatmap() {
  ...
  const getColor = (intensity) => {
    if (intensity < 0.15) return 'bg-white/[0.02]';
    if (intensity < 0.35) return 'bg-indigo-500/20';
    if (intensity < 0.55) return 'bg-indigo-500/40';
          ...
          {data.map((week, wi) => (
            <div key={wi} className="flex flex-col gap-1">
              {week.map((intensity, di) => (
                <div
                  key={`${wi}-${di}`}
                  className={`w-3 h-3 rounded-[3px] ${getColor(intensity)} transition-all duration-300 hover:ring-1 hover:ring-indigo-400/30 cursor-pointer`}
                  title={`$${(intensity * 500).toFixed(0)} spent`}
                />
              ))}
            </div>
          ))}
