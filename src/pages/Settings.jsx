...
export default function Settings() {
  ...
  const { user, currency, setPreferredCurrency, fetchAllData } = useFinance();

  // Profile — seeded from real API user, editable locally (future: PATCH /api/auth/profile)
  const [profileName, setProfileName] = useState(user?.name || '');
  const [profileEmail, setProfileEmail] = useState(user?.email || '');

  // Keep form in sync if user object loads after mount
  useEffect(() => {
  ...
  }, [profileName, profileEmail, user, fetchAllData]);
        ...
        {activeTab === 'profile' && (
          <div className="p-6 space-y-6">
            ...
            >
              ...
              <span>{savedFlash ? '✓ Saved!' : 'Save Changes'}</span>
            </button>
          </div>
        )}
